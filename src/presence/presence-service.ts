...
export class PresenceService {
  ...
  async startEventForwarder(io: SocketIOServer): Promise<void> {
    if (this.subscriber) {
      return;
    }
    ...
    subscriber.on("pmessage", (_pattern, _channel, message) => {
      try {
        ...
        io.to(payload.roomId).emit("presence:event", payload);
      } catch (error) {
        this.logger.error("Failed to process presence event", error);
      }
    });
  }
